...
export default function Projects() {
  const { tasks, setTaskComplete, addTask, updateTask, deleteTask, moveTask } = useTasks()
    ...
    >
      <Box sx={{ flex: '1 1 60%', minWidth: 0, overflow: 'hidden', p: 2, borderRight: '1px solid var(--scratchpad-separator)', display: 'flex', flexDirection: 'column' }}>
        ...
        <Box sx={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column' }}>
          <TasksPanel tasks={tasks} projectId={selectedProjectId} onTaskComplete={setTaskComplete} addTask={addTask} updateTask={updateTask} deleteTask={deleteTask} moveTask={moveTask} />
        </Box>
      </Box>
