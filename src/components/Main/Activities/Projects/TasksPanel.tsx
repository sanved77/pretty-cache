...
function getProjectTaskIds(tasks: Task[], projectId: string): Set<string> {
  ...
  function add(task: Task) {
    ...
    for (const id of task.subTasks ?? []) {
      const child = byProject.find((t) => t.id === id) ?? tasks.find((t) => t.id === id)
      if (child) add(child)
    }
  }
...
function getProjectCompletion(tasks: Task[], projectId: string, taskMap: Map<string, Task>): { completed: number; total: number } {
...
function sortTasksIncompleteFirst(tasks: Task[], taskMap: Map<string, Task>): Task[] {
...
export default function TasksPanel({ tasks, projectId, onTaskComplete, addTask }: TasksPanelProps) {
  ...
  const { completed, total } = useMemo(() => getProjectCompletion(tasks, projectId, taskMap), [tasks, projectId, taskMap])
